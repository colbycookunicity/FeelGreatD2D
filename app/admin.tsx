...
export default function AdminScreen() {
  ...
  const [formRole, setFormRole] = useState<"admin" | "sales_rep">("sales_rep");
    ...
    return (
      <View style={[styles.userCard, { backgroundColor: theme.surface, opacity: isActive ? 1 : 0.6 }]}>
        <Pressable style={styles.userCardContent} onPress={() => openEditModal(item)}>
          <View style={[styles.avatar, { backgroundColor: item.role === "admin" ? theme.accentSecondary + "20" : theme.tint + "20" }]}>
            <Ionicons
              name={item.role === "admin" ? "shield" : "person"}
              size={20}
              color={item.role === "admin" ? theme.accentSecondary : theme.tint}
            />
          </View>
          <View style={styles.userInfo}>
            ...
            <Text style={[styles.userMeta, { color: theme.textSecondary }]}>
              @{item.username} {item.role === "admin" ? " - Admin" : " - Sales Rep"}
            </Text>
          </View>
          <View style={styles.userActions}>
            ...
            {isSelf && (
              <View style={[styles.selfBadge, { backgroundColor: theme.tint + "20" }]}>
                <Text style={[styles.selfBadgeText, { color: theme.tint }]}>You</Text>
              </View>
            )}
          </View>
        </Pressable>
                ...
                <View style={styles.roleRow}>
                  <Pressable
                    style={[
                      styles.roleBtn,
                      {
                        backgroundColor: formRole === "sales_rep" ? theme.tint : theme.background,
                        borderColor: theme.border,
                      },
                    ]}
                    onPress={() => setFormRole("sales_rep")}
                  >
                    <Text style={{ color: formRole === "sales_rep" ? "#FFF" : theme.text, fontFamily: "Inter_500Medium", fontSize: 14 }}>
                      Sales Rep
                    </Text>
                  </Pressable>
                  <Pressable
                    style={[
                      styles.roleBtn,
                      {
                        backgroundColor: formRole === "admin" ? theme.accentSecondary : theme.background,
                        borderColor: theme.border,
                      },
                    ]}
                    onPress={() => setFormRole("admin")}
                  >
                    <Text style={{ color: formRole === "admin" ? "#FFF" : theme.text, fontFamily: "Inter_500Medium", fontSize: 14 }}>
                      Admin
                    </Text>
                  </Pressable>
                </View>
